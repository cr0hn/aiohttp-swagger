...
@swagger_validation
async def post1(request, *args, **kwargs):
    """
    ---
...
@swagger_validation
async def post2(request, *args, **kwargs):
    """
    ---
    description: Post resources
    tags:
...
@pytest.mark.parametrize("method,url,body,headers,response",
                         POST1_METHOD_PARAMETERS)
async def test_function_post1_method_body_validation(
...
@pytest.mark.parametrize("method,url,body,headers,response",
                         POST2_METHOD_PARAMETERS)
async def test_function_post2_method_body_validation(
        test_client, loop, swagger_file, method, url, body, headers, response):
    app = web.Application(loop=loop)
